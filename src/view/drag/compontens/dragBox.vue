        ...
        <template v-for="(item, i) of formList">
            <div class="itemBox">
                <div class="box" :class="[item.id === store.currentClickItem.id ? 'action' : '']">
                    <formEl :formItem="item" type="autocomplete" v-if="item.type == 'autocomplete'"
                        source="dragContentBox" :id="item.id">
                        <el-autocomplete clearable class="inline-input w-50" placeholder="Please Input" />
                    </formEl>
                    <formEl :formItem="item" type="input" v-if="item.type == 'input'"
                        source="dragContentBox" :id="item.id">
                        <el-input v-model="item.data.defaultVal" :placeholder="item.data.placeholder" />
                    </formEl>
                    <formEl :formItem="item" type="cascader" v-if="item.type == 'cascader'"
                        source="dragContentBox" :id="item.id">
                        <el-cascader />
                    </formEl>
                    <formEl :formItem="item" type="checkbox" v-if="item.type == 'checkbox'"
                        source="dragContentBox" :id="item.id">
                        <el-checkbox label="Option 1" size="large" />
                    </formEl>
                    <formEl :formItem="item" type="color-picker" v-if="item.type == 'color-picker'"
                        source="dragContentBox" :id="item.id">
                        <el-color-picker />
                    </formEl>

                    <formEl :formItem="item" type="date-picker" v-if="item.type == 'date-picker'"
                        source="dragContentBox" :id="item.id">
                        <el-date-picker type="daterange" unlink-panels range-separator="To" start-placeholder="Start date"
                            end-placeholder="End date" />
                    </formEl>
                    <formEl :formItem="item" type="radio" :source="'dragContentBox'"
                        v-if="item.type == 'radio'">
                        <el-radio-group>
                            <el-radio>Option A</el-radio>
                        </el-radio-group>
                    </formEl>
                    <formEl :formItem="item" type="rate" :source="'dragContentBox'"
                        v-if="item.type == 'rate'" :id="item.id">
                        <el-rate clearable />
                    </formEl>

                    <formEl :formItem="item" type="slider" :source="'dragContentBox'"
                        v-if="item.type == 'slider'" :id="item.id">
                        <el-slider show-input />
                    </formEl>
                    <formEl :formItem="item" type="switch" :source="'dragContentBox'"
                        v-if="item.type == 'switch'" :id="item.id">
                        <el-switch />
                    </formEl>
                    <formEl :formItem="item" type="time-picker" :source="'dragContentBox'"
                        v-if="item.type == 'time-picker'" :id="item.id">
                        <el-time-picker placeholder="Arbitrary time" />
                    </formEl>
                    <formEl :formItem="item" type="time-picker" :source="'dragContentBox'"
                        v-if="item.type == 'transfer'" :id="item.id">
                        <el-transfer filterable filter-placeholder="State Abbreviations" />
                    </formEl>
                    <formEl :formItem="item" type="upload" :source="'dragContentBox'"
                        v-if="item.type == 'upload'" :id="item.id">
                    ...
                    <formEl :formItem="item" type="editor" :source="'dragContentBox'"
                        v-if="item.type == 'editor'" :id="item.id">
                ...
                <div class="buttons" v-if="item.id === store.currentClickItem.id">
                    ...
                    <icon @click="del" icon="material-symbols:delete-outline" color="#fc6f6f" style="border:solid 1px #ccc;" />
                </div>
            </div>
        </template>
    </div>
